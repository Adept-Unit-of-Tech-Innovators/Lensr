...
public class SphericalLens extends Group implements Glass, Editable, Serializable {
    ...
    public void create() {
        ...
        for (Shape element : elements) {
            element.setStroke(LensrStart.mirrorColor);
            element.setStrokeWidth(LensrStart.globalStrokeWidth);
            ...
            if (element instanceof LensArc arc) {
                ...
                arcVertex.setOnClickEvent(event -> LensrStart.taskPool.execute(() -> {
                    while (LensrStart.isMousePressed) {
                        arc.scale(LensrStart.mousePos);
            ...
            if (element instanceof LensLine line) {
                ...
                if (Math.abs(rotationDelta) == 0) {
                    ...
                    if (arc.getAnchors().a().equals(anchor) || arc.getAnchors().b().equals(anchor)) {
                        ...
                        if (arc == elements.get(2))
                            arc1Vertex = new Point2D(arc.getVertex().getX(), arc.getVertex().getY());
                        else arc2Vertex = new Point2D(arc.getVertex().getX(), arc.getVertex().getY());
                        continue;
                    }
                ...
                Point2D relativeVertex = Intersections.rotatePointAroundOtherByAngle(arc.getVertex(), new Point2D(centerX, centerY), rotationDelta);
    ...
    public void move() {
        LensrStart.taskPool.execute(() -> {
            while (LensrStart.isMousePressed) {
                double x = LensrStart.mousePos.getX() - centerX;
                double y = LensrStart.mousePos.getY() - centerY;
    ...
    public void scale(Point2D anchor) {
        LensrStart.taskPool.execute(() -> {
            ...
            while (LensrStart.isMousePressed) {
                newCenterX = LensrStart.mousePos.getX() + (anchor.getX() - LensrStart.mousePos.getX()) / 2;
                newCenterY = LensrStart.mousePos.getY() + (anchor.getY() - LensrStart.mousePos.getY()) / 2;

                Point2D centerPoint = new Point2D(newCenterX, newCenterY);
                Point2D unrotatedMousePos = Intersections.rotatePointAroundOtherByAngle(LensrStart.mousePos, centerPoint, -angleOfRotation);
                Point2D unrotatedAnchorPos = Intersections.rotatePointAroundOtherByAngle(anchor, centerPoint, -angleOfRotation);
    ...
    public void rotate() {
        LensrStart.taskPool.execute(() -> {
            ...
            double mouseAngle = Math.atan2(centerY - LensrStart.mousePos.getY(), centerX - LensrStart.mousePos.getX());
            while (LensrStart.isMousePressed) {
                int snapValue = 1;

                newAngleOfRotation = Math.atan2(centerY - LensrStart.mousePos.getY(), centerX - LensrStart.mousePos.getX()) - mouseAngle;
                if (LensrStart.shiftPressed) snapValue = 45;
                else if (LensrStart.altPressed) snapValue = 90;
    ...
    public boolean intersectsMouseHitbox() {
        ...
        return Shape.intersect(bounds, LensrStart.mouseHitbox).getLayoutBounds().getWidth() != -1;
    }
