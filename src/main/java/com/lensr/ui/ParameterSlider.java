...
public class ParameterSlider extends JFXSlider {
    ...
    public ParameterSlider(Object source, ValueToChange valueToChange, SliderStyle sliderStyle) {
        ...
        hBox.setViewOrder(-1);
        LensrStart.root.getChildren().add(hBox);
        ...
        valueProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (valueToChange == ValueToChange.NumberOfRays) {
                roundedValue = newValue.intValue();
            ...
            if (currentSource instanceof BeamSource beamSource && valueToChange == ValueToChange.NumberOfRays) {
                beamSource.setRayCount((int) roundedValue);
            }
            if (currentSource instanceof PointSource pointSource && valueToChange == ValueToChange.NumberOfRays) {
                // why +1? idk but it works
                pointSource.setRayCount((int) roundedValue+1);
            }
